...
bool ImGui::BeginMenuBackground(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
	...
	if (first_begin_of_the_frame)
	{
		...
		window->SizeContents = CalcContentSize(window);
		...
		if (!(flags & ImGuiWindowFlags_NoTitleBar))
		{
			...
			if (p_open != NULL)
			{
				...
				if (CloseButton(window->GetID("#CLOSE"), ImVec2(window->Pos.x + window->Size.x - style.FramePadding.x - rad, window->Pos.y + style.FramePadding.y + rad)))
					* p_open = false;
			}
